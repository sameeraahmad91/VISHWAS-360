...
Supabase Auth owns credentials and this API owns marketplace data. The service
role key is used only server-side; clients receive a short-lived access token
and the profile needed by the Streamlit UI.
"""
...
app = FastAPI(title="VISHVAS 360 API", description="Trusted local service marketplace", version="2.1.0")
...
class SignupIn(AuthCredentials):
...
class PaymentIn(BaseModel):
    booking_id: int; customer_id: str; amount: float = Field(gt=0); payment_method: str = "upi"
class ComplaintIn(BaseModel):
    booking_id: Optional[int] = None; customer_id: str; provider_id: Optional[int] = None
    subject: str = Field(min_length=3); description: str = Field(min_length=5)
...
def auth_error(exc: Exception) -> HTTPException:
    ...
    return HTTPException(400, message or "Authentication request failed")


def profile_for(user_id: str) -> dict[str, Any]:
    profile = db("profiles").select("*").eq("id", user_id).single().execute().data
    if not profile:
        raise HTTPException(500, "Authenticated user has no profile")
    return profile


def auth_result(user: Any, session: Any) -> dict[str, Any]:
    if not user:
        raise HTTPException(400, "Supabase did not return a user")
    return {
        "success": True,
...
def signup(payload: SignupIn):
    try:
        ...
        raise auth_error(exc)
    user = getattr(result, "user", None)
    session = getattr(result, "session", None)
    if not user:
        raise HTTPException(400, "Unable to create account")
    try:
        db("profiles").insert({"id": user.id, "full_name": payload.full_name,
            "phone": payload.phone, "role": payload.role, "language": payload.language}).execute()
        if payload.role == "provider":
            db("service_providers").insert({"profile_id": user.id, "verification_status": "pending",
                                              "availability_status": "offline"}).execute()
    except Exception as exc:
        # Auth users cannot be safely rolled back through all Supabase versions;
        # report the actionable error rather than returning a partially valid login.
        raise HTTPException(500, f"Account created but profile setup failed: {exc}")
    return auth_result(user, session)
...
def login(payload: AuthCredentials):
    try:
        ...
        raise auth_error(exc)
    return auth_result(getattr(result, "user", None), getattr(result, "session", None))
...
def distance_km(a_lat, a_lon, b_lat, b_lon) -> float:
    if None in (a_lat, a_lon, b_lat, b_lon): return float("inf")
    p1, p2 = radians(a_lat), radians(b_lat)
    h = sin(radians(b_lat-a_lat)/2)**2 + cos(p1)*cos(p2)*sin(radians(b_lon-a_lon)/2)**2
...
def trust_score(provider_id: int) -> int:
    ...
    return round(min(100, verified + rating + min(completed / 50, 1) * 20 + min((provider.get("experience_years") or 0) / 10, 1) * 10 + max(0, 15 - complaints * 3)))
...
@app.get("/")
def home(): return {"name": "VISHVAS 360", "status": "running", "version": app.version, "utc": datetime.now(timezone.utc)}

@app.get("/health")
def health():
    try: db("service_categories").select("id").limit(1).execute(); return {"status": "healthy", "database": "connected"}
    except Exception as exc: return {"status": "degraded", "database": "unavailable", "detail": str(exc)}

@app.get("/services")
def get_services(): return {"success": True, "services": db("service_categories").select("*").order("name").execute().data or []}

@app.get("/providers")
def get_providers(service_id: int | None = None, verified_only: bool = True, latitude: float | None = Query(default=None, ge=-90, le=90), longitude: float | None = Query(default=None, ge=-180, le=180), radius_km: float = Query(default=50, gt=0, le=500)):
    ...
    result = []
    for p in providers:
        item = enrich_provider(p); item["distance_km"] = distance_km(latitude, longitude, p.get("latitude"), p.get("longitude"))
        if item["distance_km"] <= radius_km or latitude is None: result.append(item)
    return {"success": True, "providers": sorted(result, key=lambda x: (x["distance_km"], -x["trust_score"]))}
...
def list_bookings(customer_id: str | None = None, provider_id: int | None = None):
    q = db("bookings").select("*").order("created_at", desc=True)
...
def create_booking(payload: BookingIn):
    ...
    result = db("bookings").insert(data).execute(); booking = result.data[0] if result.data else data
    notify(payload.customer_id, "Booking placed", f"Request #{booking.get('id')} has been placed.")
    if booking.get("provider_id"):
        p = db("service_providers").select("profile_id").eq("id", booking["provider_id"]).single().execute().data
        if p: notify(p["profile_id"], "New booking request", f"A customer requested service (#{booking.get('id')}).", "new_booking")
...
def get_notifications(user_id: str): return {"success": True, "notifications": db("notifications").select("*").eq("user_id", user_id).order("created_at", desc=True).limit(50).execute().data or []}
@app.patch("/notifications/{notification_id}/read")
def mark_read(notification_id: int): db("notifications").update({"is_read": True}).eq("id", notification_id).execute(); return {"success": True}
...
def create_payment(payload: PaymentIn):
    ...
    result = db("payments").insert(data).execute(); notify(payload.customer_id, "Payment received", f"₹{payload.amount:.0f} paid for booking #{payload.booking_id}.", "payment")
...
def create_review(payload: ReviewIn):
    ...
    result = db("reviews").insert(data).execute(); return {"success": True, "review": result.data[0] if result.data else data}

@app.post("/complaints")
def create_complaint(payload: ComplaintIn):
    data = payload.model_dump(); result = db("complaints").insert(data).execute(); return {"success": True, "complaint": result.data[0] if result.data else data}

@app.post("/assistant")
def assistant(payload: AssistantIn):
    result = detect_intent(payload.message, payload.history); providers = []
    if result.get("service_category"):
        cats = db("service_categories").select("id").eq("name", result["service_category"]).execute().data or []
        if cats: providers = get_providers(service_id=cats[0]["id"])["providers"]
    return {"success": True, **result, "provider_count": len(providers)}
...
def admin_stats():
    ...
    return {"success": True, "stats": {"profiles": count("profiles"), "providers": count("service_providers"), "bookings": count("bookings"), "revenue": sum(float(p["amount"]) for p in payments)}}
